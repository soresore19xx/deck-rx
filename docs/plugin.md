# Deck RX — the Stream Deck + plugin

The original of the four: fourteen actions and the encoder dials, driving a remote SpyServer from the deck itself.

Part of [deck-rx](../README.md).

![all four LCD panels](lcd-combined.png)

Per-dial layouts and screenshots — Tune, Volume, Combo, FM, AM, SSB, Band
Select, Options and the FFT display — are in [dial-layouts.md](dial-layouts.md).
...
| Remote SDR | Any SpyServer-compatible receiver. Tested with **Airspy HF+ Discovery** (HF 0.5–31 MHz + VHF 60–260 MHz, 31–60 MHz hardware gap) running SpyServer on a Linux ARM/aarch64 (NanoPi etc.); see [docs/server-setup.md](server-setup.md) for the server side. Airspy R2 / Mini and RTL-SDR also expected to work but are less exercised. |

Developer / contributor tooling (Node 20+, MacPorts `librsvg` / `ImageMagick` / `sox` for the dump + analysis scripts) is documented in [docs/build-install.md](build-install.md). The native receiver app additionally needs `swiftc` (Xcode Command Line Tools); `rsvg-convert` renders its icon from the plugin's own `imgs/icon-source.svg` when present, and without it the app falls back to the generic bundle icon.
...
| FM Bandwidth | ✅ user-cycle 250 / 200 / 150 / 110 / 100 / 90 kHz (JP 100 kHz channel-spacing-aware). The width also decides how much stereo survives — measured channel separation on a full-deviation broadcast MPX is 62 dB at 250 kHz, 36 at 200, 30 at 150 and 11 at 110, where the image is gone well before the subcarrier is; complex IF LPF on I/Q is a Blackman-windowed sinc FIR (linear phase, ~−74 dB stopband past cutoff). Replaced the earlier 8th-order Butterworth IIR, which only reached ~−24 dB at the narrowest cycle setting and let neighbour stations leak through the discriminator |
| AM/SW Bandwidth | ✅ (16th-order complex IF LPF on I/Q at half the channel width + 8th-order post-envelope LPF, **both** at half the width). The post-envelope filter used to be cut at the full channel width, which left the 4.5-9 kHz octave of a 9 kHz channel riding on the IF skirt alone — measured against SDR++ on 594 kHz that octave sat ~25 dB down where SDR++ was at −85. Halving it is worth 15 dB and is now in place. `Deck RX Solo` follows its own halving with a brick-wall FIR ([solo.md](solo.md#the-am-channel-filter)) that Node cannot afford, so above the transition Solo is still the quieter of the two |
...
| FM Stereo PLL (true phase lock) | ✅ (2nd-order Costas-style PLL locks VCO to 19 kHz pilot, loop bandwidth ≈ 50 Hz, damping = 1/√2, hysteretic lock detection so weak/intermittent pilots don't flap; STEREO badge shown only while pilot-locked AND user has stereo option enabled). 8th-order Butterworth audio LPF (4 cascaded biquads) at 15 kHz on both L+R and L−R recovery paths kills the 19 kHz pilot residue and 23-53 kHz DSB-SC subcarrier so they never reach the DAC. The 38 kHz reference is built from the phase the pilot was measured against rather than from the phase the loop has already advanced — one sample is 15° at 19 kHz on a 456 kHz stream, doubled to 30° at 38 kHz, which scaled L−R by cos 30°. It is also the **sine** of that doubled phase (`−2·sinV·cosV`), which the standard requires and which this decoder got wrong until 2026-09-12: `cos(2·phase)` is in quadrature with a real broadcast's subcarrier, so the coherent L−R was zero and every station came out mono. Separation on a standard-compliant composite: 0.0 dB before, 48 dB after. |
...
| Master OFF releases the device | ✅ the Tune dial's 2 s long press tears the SpyServer connection down, and the socket goes first — `stopAudio()` used to be awaited ahead of it, so one rejection there left the receiver reading OFF everywhere while still holding the radio away from everything else on the machine. While OFF nothing dials out: `scheduleReconnect` returns early and `connect()` stays offline. The Tune LCD shows `-----` for the frequency, the same as a dropped link, with the header saying which of the two it is (`OFF` or `LINK`) |
| Shared device, control regained | ✅ SpyServer gives control to the first client and drops the others' retunes without a word; when that client leaves, whoever is left is promoted. The plugin acts on that edge — it keeps the frequency it was told to be on through every refusal and re-issues it the moment it may, logging `control regained — retuning to <hz>`. Without it the receiver sat on the band the departed client had left the device on, indistinguishable from a dead radio. While control is elsewhere the Tune dial's header reads **LISTEN ONLY** in place of the station name, so "the dial does nothing" has somewhere to be read |
...
| Spectrum feed | ✅ binary FFT frames over a Unix socket (`/tmp/deck-rx-spectrum.sock`, `src/spectrumFeed.ts`) for a native front-end. Computes nothing while nobody is connected, and drops frames for a reader that falls behind rather than queueing them. See [Spectrum feed](native-app.md#spectrum-feed-native-front-end). |
...
| `GET /options[?set=<name>&value=<v>]` | Read the demod's own settings (FM / AM / SSB groups plus RF gain), or change one by name — `fm.stereo`, `am.sync`, `ssb.bandwidth`, `gain`. Always answers with everything in force. `gain` is one control over two stored values: it writes `amGain` in AM and `fmGain` in every other mode, which is the split the audio path itself makes — SSB and CW ride the FM value, so a gain set while listening to either has to move that one. The value is filed under the band being listened to (mw / hf / vhf), so it comes back when that band does |
