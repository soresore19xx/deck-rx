...
| Auto station-name lookup | ✅ **Japan-area only** for the JP DB — region-switchable from the Tune dial PI (関東 / 北海道 / 近畿 / 中国 / 九州 / 沖縄 ※現状スクレイパー実装は**関東のみ**、他は順次対応) + region-independent manual overrides; the EIBI SW DB covers international shortwave (day/time/spur-aware); in-PI `Update Now` for both |
