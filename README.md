...
- **Deck RX Combo Options** (alternative to the two single-mode panels) — both AM and FM options on one LCD, side-by-side at row-height 12 px / font 10. Left column is AM (BW / CAGC / Sync / Atk / Dec / Gain / Mode), right column is FM (Deemph / IFNR / HPF / LPF / Ste / Gain / Step). The active column auto-tracks the current demod mode (AM mode = mode 2 lights the left, otherwise the right) and is marked with a `▶` after the column header; the inactive column shows live values in dim grey for awareness but isn't editable from this dial. Rotate navigates inside the active column, push toggles edit mode, edits operate on the active column's selected row only. The two single-mode panels are kept registered so existing Stream Deck profiles continue to work — pick whichever layout suits your slot budget.
