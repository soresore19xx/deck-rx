...
- **Audio sink: `naudiodon` for local, `ffmpeg` for icecast publish.** Local audio output goes through naudiodon (PortAudio binding) directly to CoreAudio's HAL — no intermediate child process, no resampling, no IPC. The icecast publish path still uses ffmpeg, because ffmpeg cleanly hosts both the MP3 encoder (libmp3lame) and the icecast SOURCE protocol client; replacing it would mean carrying a JS-side encoder + an HTTP-PUT-with-source-headers implementation, which is well out of scope. See "Why naudiodon, not ffmpeg → audiotoolbox" below for the motivation.
- Audio device selection by **name** (resilient against device id renumbering when USB devices come and go; both PortAudio and CoreAudio enumerate devices in plug order). naudiodon's `getDevices()` returns the same CoreAudio names as `SwitchAudioSource -t output -a`, so the PI's dropdown and the persisted `naudiodon.deviceName` round-trip cleanly.
...
## Audio sink: naudiodon vs ffmpeg → audiotoolbox

Earlier deck-rx releases used a single audio path: PCM was piped to a
spawned `ffmpeg -f audiotoolbox` child process which then handed bytes to
CoreAudio. That worked, but accumulated three structural problems over
hours of continuous playback:

1. **5-hour audiotoolbox wedge.** ffmpeg's `audiotoolbox` muxer stops
   draining its internal buffer after ~5 h of uninterrupted use (the
   exact mechanism is upstream-side and not pinned down — it reproduces
   on both ffmpeg 4.x and 7.x, with multiple device combinations). ffmpeg
   keeps accepting stdin writes, exits cleanly when sent SIGTERM, but no
   audio reaches the device. The user notices a silent radio with no
   error in the log. A respawn-on-exit watchdog doesn't help because the
   process is still alive.

2. **Async resampler queue creep.** ffmpeg's `aresample=48000:async=...`
   absorbs the ppm-level clock drift between the Airspy's crystal and
   the CoreAudio output device's crystal by queueing samples in an
   internal buffer. Over 10+ minutes the queue grows by 50–200 ms per
   hour, which is fine when you're just listening but shows up as a
   visible "preset switch lag grows the longer the plugin runs"
   symptom — user rotates the dial, the new station's first second is
   trailing the queued tail of the old station.

3. **Implicit resampler-LPF coupling.** ffmpeg's aresample bakes in an
   anti-alias FIR LPF around 24 kHz. Several demodulators were
   accidentally relying on that filter — the WFM stereo decoder's 19
   kHz pilot residue, the 23–53 kHz L−R DSB-SC subcarrier, the CW
   pre-decimation noise floor — and the upstream code never had to
   defend against them. Replacing ffmpeg with a sink that doesn't
   resample exposed every one of those, audibly: 19 kHz whine on FM
   stereo, distorted L−R, noisier CW. The fix was to add explicit
   per-demod LPFs (8th-order Butterworth at 15 kHz for WFM stereo,
   4th-order Butterworth pre-decimation for CW, etc.), but identifying
   that those were latent bugs only happened when the implicit safety
   net was removed.

`naudiodon` (a Node binding for [PortAudio](http://www.portaudio.com/))
fixes all three:

- **No external process** — PortAudio runs in-process. The wedge mode
  doesn't reproduce; even if it did, there's no opaque child to lose
  state inside.
- **No resample** — the demodulator's natural audio rate (114 kHz for
  AM, varies by mode) is fed straight to CoreAudio HAL. The output
  device negotiates its own internal rate. Clock drift becomes a few
  μs of jitter that the HAL handles transparently, not a multi-second
  queue.
- **Forces explicit LPF discipline** — every demod path is now
  responsible for its own band-shaping. The 8th-order Butterworth
  output-stage LPF in `NaudiodonOutput` itself (cutoff 22 kHz, 4
  cascaded biquads × 2 channels) is a safety net against future demods
  that forget, but per-mode filters in `demodulator.ts` do the real
  work.

The trade-offs we accepted to switch:

- **Native binding requires an ABI-matched rebuild.** Stream Deck's
  bundled Node has its own ABI version, separate from the dev machine's
  Node. The build script (`scripts/rebuild-naudiodon.sh`) handles this,
  but it's a manual step after `npm install`. See
  [docs/build-install.md](build-install.md) for the procedure.
- **No free MP3 / network publishing.** ffmpeg was doing double duty as
  the local audio sink AND the icecast SOURCE encoder. With ffmpeg
  removed from the local path, the codebase keeps `FfmpegOutput` for
  the icecast publish path only — selected at start time when
  `cfg.ffmpeg.mode === 'icecast'`. PCM gets resampled to 48 kHz, MP3-
  encoded by libmp3lame, and PUT to the icecast mount over HTTP. The
  5 h wedge bug doesn't apply here because icecast's network sink isn't
  audiotoolbox.
- **Slightly different perceived volume.** ffmpeg's resampler had a
  small amount of headroom built in; naudiodon writes the demodulator's
  Int16 stream verbatim. Users coming from the ffmpeg path may want to
  drop Volume by ~10 % to match what they were used to.
...
- **Audio device by name**: PortAudio device ids also change when USB devices are added or removed. We persist the device **name** (`naudiodon.deviceName` in `config.json`) and resolve the current id via `naudiodon.getDevices()` every time `NaudiodonOutput.start()` runs, falling back to PortAudio default (id `-1`) if the named device is not present. Some USB DACs (Topping DX7s, SMSL USB AUDIO, …) include a trailing space in the CoreAudio name — the resolver trims both sides so a lookup like `"DX7s"` matches `"DX7s "`. (The icecast/ffmpeg path doesn't need a device name; it's network-only.)
- **Pop suppression**: the iq listener writes silent PCM whenever `Date.now() < muteUntil`. Four independent paths set `muteUntil`:
  - startup (500 ms);
  - **preset retune** / band-fallback / connect-time seed (`setFrequency(..., smooth: false)`): **100 ms for FM/NFM/WFM/SSB/CW, 200 ms for AM**. AM uses a longer window because `demod.resetForRetune()` zeros the carrier AGC (`amDc / amAgcAmp / amSyncPhase`), which needs ~150–200 ms to re-converge to the new station's carrier level — 100 ms left a residual level step audible as ボツッ;
  - **VFO smooth retune** (`setFrequency(..., smooth: true)`): no mute on FM/NFM/WFM/SSB/CW, 200 ms on AM (AM sync PLL re-lock window, set to match the wn=150 Hz pull-in time);
  - **gain change** (200 ms initial + 150 ms re-mute on apply, both with `demod.reset()`).
  Both retune and gain change use an 80–120 ms debounce so rapid dial rotation groups into a single SpyServer command + one mute window. The demodulator skips `atan2(0,0)` when both vectors are near zero.
- **Mute-boundary fade ramp**: zero-filling at `muteUntil` produces a sharp amplitude step (old PCM at full volume → 0), and the matching step at mute lift (0 → new PCM at full volume). On the naudiodon path, PortAudio's queue (~145 ms cushion at maxQueue=4) delays the audible boundary so these steps fall outside the mute window — listeners hear them as ボツッ clicks separated by a brief silence. The iq listener applies an **8 ms linear fade-out** on the first muted buffer and **8 ms linear fade-in** on the first un-muted buffer, so the boundary becomes a soft envelope instead of a hard step. The ramp is applied in addition to (not in place of) the existing mute window. `lastBufferMuted` is the single bit of state the listener carries between calls to detect the transition.
- **naudiodon `maxQueue` sizing**: PortAudio's writer-side buffer depth is a tradeoff. Too thin (default 2 buffers ≈ 72 ms at 114 kHz × 4096-sample packets) → SpyServer ↔ DX7s clock-drift between writer rate and CoreAudio reader rate causes underruns that play out as intermittent silence frames ("ビリビリ"). Too deep (8 buffers ≈ 290 ms) → the queue holds nearly 300 ms of old-freq PCM after a preset change, so the post-mute boundary falls INSIDE the queue and the mute window doesn't bridge it (this was the regression we hit moving from ffmpeg to naudiodon — ffmpeg's internal queue was much shallower). Current setting: **4 buffers ≈ 145 ms**, comfortably close to the 100/200 ms preset-mute window so the boundary clears the queue before the next station's audio arrives.
...
- **Audio device routing**: `naudiodon.getDevices()` returns CoreAudio output devices with their HAL display name, default sample rate, and PortAudio device id. The PI populates its dropdown from `SwitchAudioSource -t output -a` (same CoreAudio name source), the user picks one, and `NaudiodonOutput.resolveDeviceId()` maps the persisted name back to an id at `start()` time. Trailing whitespace common to USB device names is trimmed on both sides of the comparison.
