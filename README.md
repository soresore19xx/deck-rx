...
| Runtime tooling (MacPorts) | `sudo port install ffmpeg switchaudio-osx` — `ffmpeg` is the demod → audio bridge, `switchaudio-osx` resolves output device names. The Stream Deck app ships its own bundled Node for running installed plugins. |
