...
| AM/SW Bandwidth | ✅ (16th-order complex IF LPF on I/Q at half the channel width + 8th-order post-envelope LPF). The post-envelope filter is cut at the **full** channel width, so the 4.5-9 kHz octave of a 9 kHz channel rides on the IF skirt alone — measured against SDR++ on 594 kHz that octave sat ~25 dB down where SDR++ was at −85. `Deck RX Solo` cuts it at half the width and follows with a brick-wall FIR ([solo.md](solo.md#the-am-channel-filter)); the same one-line halving is worth 15 dB here and has not been made |
